...
foreach lines $lib_dir {	
	...
	read_liberty $lines
		...
		if {[file exists $preroute_file_path]} {
			set filename3 [lindex [split [lindex [split [file rootname [file tail $preroute_file_path] ] .] 1] _] 1] 
			set filename4 [string map {"preroute" "Layout"} $filename3]
			mkdir -p Post_$filename4
			mkdir -p Post_$filename4\_WNS
			mkdir -p Post_$filename4\_TNS
